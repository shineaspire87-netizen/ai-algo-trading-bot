# train_model_institutional.py - Bug Fixed Institutional AI Trainer
import os
...
def prepare_institutional_dataset(ticker: str = "^NSEBANK", period: str = "60d", interval: str = "5m") -> pd.DataFrame:
    ...
    print("2. 🔬 Computing 20+ Institutional Technical & Regime Indicators...")
    df['RSI'] = ta.momentum.rsi(df['Close'], window=14)
    df['EMA_9'] = ta.trend.ema_indicator(df['Close'], window=9)
    df['EMA_21'] = ta.trend.ema_indicator(df['Close'], window=21)
    df['EMA_Diff'] = (df['EMA_9'] - df['EMA_21']) / df['EMA_21']
    
    # Official Bug-Free ADX Calculation
    ...
    dynamic_threshold = df['ATR'] * 1.0
    
    conditions = [
        (price_delta > dynamic_threshold),
        (price_delta < -dynamic_threshold)
    ]
...
def train_institutional_model():
    print("=================================================================")
    print("🤖 INSTITUTIONAL QUANT AI MODEL TRAINING (ZERO-OVERFITTING) 🤖")
    print("=================================================================")
    
    df = prepare_institutional_dataset()

    features = [
        'RSI', 'EMA_Diff', 'ADX', 'ATR_Pct', 'YZ_Volatility', 'VWAP_Diff', 
    ...
    print("4. 🛡️ Running 5-Fold TimeSeriesSplit (Purged Walk-Forward Cross Validation)...")
    ...
    for train_index, test_index in tscv.split(X):
        ...
        print(f"   ▶ Fold {fold} Out-of-Sample Accuracy: {acc * 100:.2f}%")
        fold += 1

    print(f"\n⭐ Mean Walk-Forward Out-of-Sample Accuracy: {np.mean(accuracies) * 100:.2f}%\n")

    model.fit(X, y)
    model.save_model("xgboost_model.json")
    print("✅ Institutional Model saved successfully as 'xgboost_model.json'!\n")
